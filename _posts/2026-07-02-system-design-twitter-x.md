...
date: 2026-07-02
tags: [System Design, Twitter, X, Social Media]
description: "Twitter/X serves 250M+ daily active users posting 600M+ tweets per day, processing a fan-in of 7K writes/sec and serving 300K reads/sec with sub-100ms latency for the home timeline."
...
Scoring: `score(doc) = TF-IDF × recency_boost × author_reputation`. Recency uses logarithmic decay: `1 + log(1 + hours_since_tweet)`. Concurrency uses read-copy-update with a per-partition write lock — reads proceed on a snapshot while the index updates.
...
|---|---|---|
| Hybrid fan-out (push/pull) with 1M threshold | Pure push or pure pull | Reduces celebrity-tweet write amplification from 100M → (1M+1) per tweet; read-time merge of ~5 ZSETs |
| Earlybird in-memory inverted index (per-partition) | Elasticsearch cluster | Single-digit-second freshness without a large ES cluster; proven at Twitter scale for 12+ years |
| Redis sorted sets for timeline cache | SQL-only timeline queries | Sub-millisecond sorted-range reads for pagination; O(1) insert for fan-out |
| Snowflake IDs for tweet_id | UUIDv4 / auto-increment | Monotonically increasing → chronological sort without a timestamp column; compact 64-bit for Redis ZSETs; embeds worker ID for no-collision partitioning |
| Kafka for async fan-out | Synchronous fan-out in the POST path | Decouples tweet write from timeline propagation; POST returns in 200ms, fan-out finishes within seconds |
| MySQL (hot) + Manhattan KV (cold) | Single store | MySQL gives strong consistency for recent writes; older tweets migrate to cheaper KV cold storage |

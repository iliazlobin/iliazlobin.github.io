...
Bitly turns long URLs into short ones and tracks every click. It looks simple — take a URL, generate a 7-character code, store the mapping, redirect on lookup. But the real challenge is analytics: every redirect must record an event without slowing the redirect. 40 billion active links, 360 million clicks a day, peak throughput over 200K redirects per second. The write path is small (~80 creates/sec); the read path handles millions of redirects per second. The system runs on ~270 backend services, and everything follows one rule: you never block a redirect on analytics.
...
```
URL {
  short_code:   string PK  ← base62-encoded ID, 7 chars
...
> [!TIP]
> Range batching is the universal answer to "I need globally unique IDs but I can't afford per-request coordination." Whether the backing store is ZooKeeper, etcd, Postgres (`SELECT nextval`), or Redis (`INCRBY`), the pattern is identical: claim a block, burn locally, refill async. The only decision is which store you already operate.
...
Layer 1: Browser cache (per client, private + max-age=90)
  TTL: 90s, hit rate: repeat clicks from the same client

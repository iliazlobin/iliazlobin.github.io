...
date: 2026-06-30
tags: [System Design, Real-time, WebSocket, Game]
description: "How online chess synchronizes 500K concurrent games at sub-200ms latency with stateful game servers on consistent hashing, atomic Redis matchmaking, move-log replay for crash recovery, and a multi-stage anti-cheat ML pipeline — a deep dive into real-time game infrastructure."
thumbnail: /images/posts/2026-06-30-system-design-online-chess.svg
---

How online chess synchronizes 500K concurrent games at sub-200ms latency with stateful game servers on consistent hashing, atomic Redis matchmaking, move-log replay for crash recovery, and a multi-stage anti-cheat ML pipeline — a deep dive into real-time game infrastructure.
...
2. Matchmaking Service looks up player's rating from Player DB (server-side, never trusts client)
3. Service creates a `MatchRequest` and inserts it into a Redis sorted set keyed by `{variant}:{time_control}`, scored by rating
4. Service runs `ZREVRANGEBYSCORE` to find the nearest-rated pending opponent within ±200 points
5. If found: a Lua script atomically removes both entries from the sorted set (prevents double-booking), creates a `Game` record, assigns a game server, returns `game_id` to both players
6. If not found: the POST is held open via long-poll; after 30 seconds without a match, rating range widens and retries
7. Both clients then open a WebSocket to the assigned game server
...
2. Player A sends `{ type: "move", from: "e2", to: "e4", moveNumber: 1 }` over WebSocket
3. Game Service validates against in-memory board state (legal move? correct player's turn? piece exists at `from`?)
4. If legal: apply move to in-memory board, stop A's clock, start B's clock
5. Write move to durable Move Log in MongoDB (before broadcasting — persist-then-tell)
6. Publish `opponentMove` event with new FEN and authoritative clock times to opponent via Redis pub/sub
7. Send `moveAck` back to mover confirming the move was accepted
8. If game ends (checkmate, stalemate, resignation, timeout): write final result, compute new ratings, publish `gameEnd`
9. Illegal move → `moveAck` with error, no state change
...
2. Rating Service fans rating updates into a Redis sorted set keyed by `rating` with `player_id` as member
3. `GET /leaderboard?limit=50` → `ZREVRANGEBYSCORE rating_set +inf -inf LIMIT 0 50` returns top 50 in O(log n)
4. `GET /players/{id}/rank` → `ZRANK rating_set {player_id}` returns exact rank in O(log n)
5. Redis sorted set is an external index, not the source of truth; periodic reconciliation recomputes from Player DB
...
2. API queries Game DB, returns full move list with FEN after each move, clock times, and result
3. Client replays moves locally from the move log — no server-side computation needed
4. Cursor-based pagination for games with hundreds of moves (correspondence chess)
...
2. Players join via `POST /tournaments/{id}/join`
3. For arena format: Tournament Service periodically triggers matchmaking waves, pairing available players using weighted maximum matching
4. For Swiss format: after each round, Tournament Service runs Fast Swiss pairing (O(n log n)), assigns pairings, creates games
5. Results feed back into tournament standings; final standings determined at tournament end
...
2. [Lichess WebSocket server (lila-ws)](https://github.com/lichess-org/lila-ws) — decoupled WS service communicating via Redis pub/sub
3. [Scalachess — immutable chess engine](https://github.com/lichess-org/scalachess) — functional, side-effect-free chess rules and move generation
4. [Irwin — anti-cheat with TensorFlow](https://github.com/lakinwecker/irwin) — dual-NN pipeline for engine detection
5. [Kaladin — CNN-based cheat insights](https://github.com/lichess-org/kaladin) — player-level pattern detection
6. [Fishnet — distributed Stockfish analysis](https://github.com/lichess-org/fishnet) — volunteer compute cluster for game analysis
7. [Fast Swiss (bbpPairings)](https://github.com/cyanfish/bbpPairings) — O(n log n) Swiss tournament pairings for 100K+ players
8. [Lichess production architecture diagram](https://raw.githubusercontent.com/lichess-org/lila/master/public/images/architecture.png) — official microservice topology
9. [HelloInterview — Online Chess system design](https://www.hellointerview.com/learn/system-design/in-a-hurry/introduction) — interview-oriented walkthrough with matchmaking, game server scaling, clock fairness, leaderboard deep dives
10. [Glicko-2 rating system (Glickman, 2001)](http://www.glicko.net/glicko/glicko2.pdf) — the mathematical foundation for rating with uncertainty and volatility

**Production data points**
- Lichess: 500K+ concurrent games peak, 12B+ stored games, 20+ microservices, single-developer team, free/donation-supported
- [Chess.com](http://Chess.com): 250M+ registered users, Java/Azul Zing JVM, proprietary architecture, freemium model
- Both platforms use Glicko-family rating systems, WebSocket for real-time play, MongoDB or MySQL as primary stores, and ML-based anti-cheat pipelines

...
date: 2026-06-30
tags: [System Design, Ride-Sharing, Real-time, Geospatial]
...
How Uber matches 170M+ riders to 5M+ drivers across 10,000+ cities in under two seconds with H3 geospatial indexing, batched bipartite (DISCO) matching, a DeepETA hybrid model, and event-sourced exactly-once dispatch — a deep dive into the system design of real-time ride-hailing.
...
Trip {
  trip_id:        uuid PK
  rider_id:       uuid CK           ← partitions a rider's history in Cassandra
  driver_id:      uuid
...
2. Rider App calls `POST /rides` with pickup H3 cell, dropoff H3 cell, and product type.
3. Gateway geo-routes to nearest regional cluster based on pickup location.
4. Ride Service queries Surge Engine: `GET surge:{pickup_h3}` from Redis → current multiplier.
5. Ride Service calls ETA Service with pickup→dropoff route → receives time/distance estimate.
6. Ride Service computes upfront fare: `base_rate + (distance × per_mile) + (time × per_min) + surge_bump`.
7. Ride Service creates Trip in Cassandra (status=REQUESTED), stores locked surge multiplier.
8. Returns `{trip_id, upfront_fare_cents, eta_seconds}` to Rider App.
...
2. DISCO consumer reads batches of ride requests within a 2–5s window per geographic zone.
3. For each batch, DISCO calls Location Service: `gridDisk(pickup_h3, k=7)` → ~127 H3 cells.
4. Location Service queries Redis: `ZRANGE loc:h3:{cell} 0 -1` on each cell → 20–50 candidate drivers.
5. DISCO calls ETA Service for each candidate's ETA to each rider's pickup.
6. DISCO builds bipartite cost matrix: `Cost(rider_i, driver_j) = ETA + surge_penalty + supply_value`.
7. Hungarian algorithm solves N×M assignment in O(n³), producing optimal rider→driver pairs.
8. DISCO publishes `trip.matched` event → Ride Service updates Trip status to MATCHED.
9. RAMEN pushes ride offer to selected driver's app via gRPC/QUIC with 15s acceptance timeout.
...
2. Gateway validates, deduplicates, and converts lat/lng → H3 res-9 cell via `geoToH3()`.
3. Gateway publishes location to Kafka topic `driver.location`, partition key = H3 cell.
4. Location Service consumes Kafka → writes to Redis: `ZADD loc:h3:{cell} {driver_id} {timestamp}` with 30s TTL.
5. Rider app opens WebSocket to Gateway: `WS /rides/{trip_id}/tracking`.
6. On driver match, Gateway subscribes to `trip:{trip_id}:position` updates.
7. Gateway queries Location Service for driver's current H3 cell + k=1 ring neighbors.
8. As new location events arrive, Gateway pushes `{lat, lng, heading, speed_mps, eta_seconds}` to Rider App.
...
2. During trip, Location Service continuously streams route polyline to Cassandra for trip record.
3. On dropoff, driver app sends `dropoff` event → Ride Service finalizes Trip (status=COMPLETED).
4. Ride Service publishes `trip.completed` event to Kafka.
5. Payment Service consumes event → reads locked fare from Cassandra → calculates final fare.
6. Payment Service checks Redis idempotency set: `SISMEMBER processed_trips {trip_id}`.
7. If not present, charges stored payment method via tokenized reference → INSERTs payment row into PostgreSQL with UNIQUE constraint on `trip_id`.
8. Payment Service publishes `payment.processed` → driver earnings updated, receipt generated.
...
2. Ride Service queries Cassandra: `SELECT * FROM trips WHERE rider_id = ? ORDER BY created_at DESC LIMIT 21`.
3. Cassandra wide-row model: `rider_id` is partition key, `created_at` is clustering column — all trips for one rider live in one partition.
4. If 21 rows returned, there's a next page → encode last `created_at` as cursor.
5. Returns paginated list with trip status, route summary, fare, driver rating.
6. Individual trip detail fetches full polyline and receipt via `GET /rides/{trip_id}`.
...
2. Flink stream processors maintain 5-minute sliding window per H3 cell: supply (distinct drivers pinging from cell) and demand (ride requests + app-opens in cell).
3. Per cell, Flink computes `ratio = demand / max(supply, 1)` every few seconds.
4. Multiplier derived from calibrated lookup table: gentle curve at low ratios, plateau at extreme ratios.
5. Spatial smoothing applied: average multiplier of cell + k=1 ring neighbors prevents price cliffs.
6. Multipliers written to Redis: `SET surge:{h3_cell} {multiplier} EX 60`.
7. When Ride Service calls `GET surge:{pickup_h3}`, the multiplier is read in <1ms.
8. Multiplier locked at booking time — rider sees upfront fare, not fluctuating surge.
...
2. [H3 GitHub Repository](https://github.com/uber/h3) — Open-source hexagonal geospatial indexing library.
3. [DeepETA: How Uber Predicts Arrival Times Using Deep Learning](https://www.uber.com/us/en/blog/deepeta-how-uber-predicts-arrival-times/) — Uber Engineering Blog, 2022. Hybrid routing + neural residual model with P95 4ms.
4. [Reinforcement Learning for Modeling Marketplace Balance](https://www.uber.com/us/en/blog/reinforcement-learning-for-modeling-marketplace-balance/) — Uber Engineering Blog, 2020. RL value function integrated into DISCO matching.
5. [Real-Time Push Platform: RAMEN](https://www.uber.com/us/en/blog/real-time-push-platform/) — Uber Engineering Blog, 2015. Push messaging architecture from SSE to gRPC/QUIC.
6. [Schemaless Part 1: MySQL Datastore](https://www.uber.com/us/en/blog/schemaless-part-one-mysql-datastore/) / [Part 2: Architecture](https://www.uber.com/us/en/blog/schemaless-part-two-architecture/) — Uber Engineering Blog, 2016. Sharded MySQL KV store.
7. [How Uber Serves Over 40 Million Reads Per Second Using an Integrated Cache](https://www.uber.com/us/en/blog/how-uber-serves-over-40-million-reads-per-second-using-an-integrated-cache/) — Uber Engineering Blog, 2021. CacheFront + Docstore read path.
8. [Disaster Recovery for Multi-Region Kafka at Uber](https://www.uber.com/jp/en/blog/kafka/) — Uber Engineering Blog, 2021. Active-active Kafka with surge pricing as case study.
9. [Enabling Seamless Kafka Async Queuing with Consumer Proxy](https://www.uber.com/us/en/blog/kafka-async-queuing-with-consumer-proxy/) — Uber Engineering Blog, 2021. Scaling Kafka consumption to 12M msg/s.
10. [Migrating Uber's Compute Platform to Kubernetes](https://www.uber.com/en-EG/blog/migrating-ubers-compute-platform-to-kubernetes-a-technical-journey/) — Uber Engineering Blog, 2024. 3M+ cores migrated from Mesos to K8s.
11. [Scaling Uber's Realtime Market Platform](https://qconlondon.com/london-2015/presentation/scaling-ubers-realtime-market-platform.html) — Matt Ranney, QCon London 2015. Original DISCO architecture reveal.
12. [Surge Pricing Solves the Wild Goose Chase](http://conferences.sigcomm.org/imc/2015/papers/p495.pdf) — Castillo, Knoepfle & Weyl, IMC 2015. Empirical proof that greedy matching fails without surge.

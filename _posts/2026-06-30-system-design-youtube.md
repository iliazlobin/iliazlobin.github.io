...
date: 2026-06-30
tags: [System Design, Video, Streaming, CDN, Storage]
description: "How YouTube ingests 500 hours of video every minute, transcodes it into a multi-bitrate ladder, and streams it globally under 2 s startup latency — a deep dive into resumable chunked uploads, a distributed transcoding pipeline, DASH/HLS adaptive bitrate, and CDN-fronted delivery."
thumbnail: /images/posts/2026-06-30-system-design-youtube.svg
---

How YouTube ingests 500 hours of video every minute, transcodes it into a multi-bitrate ladder, and streams it globally under 2 s startup latency — a deep dive into resumable chunked uploads, a distributed transcoding pipeline, DASH/HLS adaptive bitrate, and CDN-fronted delivery.
...
2. [Google Cloud Storage — Multipart Uploads](https://cloud.google.com/storage/docs/parallel-composite-uploads) — Composable object semantics, signed URLs, part management
3. [YouTube DNN Recommendation (Covington et al., RecSys 2016)](https://research.google/pubs/pub45530/) — Two-stage deep neural network: candidate generation via extreme multiclass classification, ranking via watch-time-weighted logistic regression
4. [Buffer-Based Rate Adaptation (Huang et al., SIGCOMM 2014)](https://dl.acm.org/doi/10.1145/2619239.2626296) — BBA algorithm: buffer occupancy → quality mapping, hybrid with throughput cap, benchmarks vs throughput-based ABR
5. [DASH Standard (ISO/IEC 23009-1)](https://www.iso.org/standard/83314.html) — MPEG-DASH manifest format, segment structure, AdaptationSet for codec switching
6. [Google Global Cache](https://peering.google.com/#/infrastructure) — ISP-peered edge caching, thousands of nodes in ISP networks globally
7. [Netflix Technology Blog — Shot-Based Encoding](https://netflixtechblog.com/optimized-shot-based-encodes-now-streaming-4b846f5ef0b0) — Scene-aware split → parallel encode → assemble pipeline, the architecture YouTube's Exo system mirrors
8. [Google SRE Book — Managing Critical State](https://sre.google/sre-book/managing-critical-state/) — Distributed consensus, erasure coding vs replication for durability at exabyte scale
9. [WebM Project / VP9](https://www.webmproject.org/vp9/) — VP9 codec specification and real-world bandwidth savings vs H.264
10. [AOM AV1 Specification](https://aomedia.org/av1/) — AV1 codec: 30% bandwidth reduction over VP9, 3–5× encode cost tradeoff

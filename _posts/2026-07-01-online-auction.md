...
date: 2026-07-01
tags: [System Design, E-Commerce, Real-Time, Auction]
description: "A marketplace transaction system where 10M auctions run concurrently with strong bid consistency, real-time updates at 5M concurrent watchers, and proxy bidding resolution — all on a Valkey Lua CAS kernel."
thumbnail: /images/posts/2026-07-01-online-auction.svg
---

Online auctions are a marketplace transaction system where 10M auctions run concurrently, each processing up to 500 bids/sec from millions of concurrent watchers — but only one bidder can win any single auction. The core tension: strong consistency on the bid path clashes with the read fan-out and scheduling demands of 10M independent timelines.
...
**Components:** API Gateway, PostgreSQL, Redis ZSET scheduler, Search index.
...
1. On any new bid arriving at the Bid Processor, before executing the Lua CAS, the processor resolves all active proxy bidders:
  ...
  - Compares `new_bid.amount` against each proxy's `max_bid`:
    - If `new_bid < highest_proxy_max`: the highest proxy wins at `new_bid + min_increment` (capped at the proxy's max). Submit a single CAS call for the proxy winner.
    - If `new_bid == highest_proxy_max`: earlier timestamp wins at the tied amount.
    - If `new_bid > all proxies`: the new bidder wins. All proxies above the new bid are outbid.
...
+-----+-----+ +-----------+ +-----------+
      | push frame to local connections
      v
+-----------+
| Client    |
| Browsers  |
...
| --- | --- | --- | --- |
| Bid consistency | Valkey Lua CAS + Kafka per-auction partition | SELECT FOR UPDATE, optimistic locking | Single-thread event loop gives sub-ms atomicity without lock manager. Database row locks queue at 500+ concurrent bidders. |
| Real-time updates | Sharded Pub/Sub + 100ms delta batching | Client polling, global Pub/Sub broadcast | O(N_gateways) work, not O(W) watchers. Polling can't meet 200ms SLA; global broadcast wastes cluster bandwidth. |
| Auction close scheduling | Sharded Redis ZSET + lease-based workers | Single DB poller, Flink keyed timers | ZSET has O(log M) poll cost; sharding eliminates single-instance bottleneck. Flink's operational overhead is disproportionate to the problem. |
| Soft close vs hard close | Soft close (60s extension, 5 max) | Hard close (fixed deadline) | Soft close eliminates sniping and keeps the auction fair. Hard close creates a game of connection latency — the bidder with the lowest ping in the last 100ms wins. |
| Proxy resolution | In-memory sort + single CAS write | N sequential CAS calls, no proxy bidding | Proxy bidding is the defining auction mechanic. Sequential CAS wastes hot-key throughput; single CAS makes proxy resolution O(1) per external bid. |
| Bid durability | Kafka (durable log) + PostgreSQL (source of truth) | Valkey-only, PostgreSQL-only | Kafka buffers spikes and survives API restarts. Valkey is the fast consistency kernel but not durable. PostgreSQL is the authority but too slow for 500 bids/sec CAS. Three layers, each for its strength. |
| Search indexing | Sharded inverted index + CDC delta pipeline | Batch rebuild (Hadoop hourly) | Delta pipeline keeps search within 30s of reality — critical for closing auctions. Batch rebuilds work for catalog but not for real-time price/status changes. |
| Settlement | Fencing token + conditional UPDATE + idempotency key | 2PC, saga without fencing | 2PC blocks on partitions and doubles latency. Fencing token is the standard pattern: INCR -> conditional WHERE fence < token -> provider-side dedup. Same guarantee, no blocking. |

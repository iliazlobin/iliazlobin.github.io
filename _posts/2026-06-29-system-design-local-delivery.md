...
1. Geo Index runs a spatial query: nearest DC with status=active within its delivery_radius_mi. Query uses a GeoHash prefix scan (7-char ≈ 150m cell) followed by a Haversine filter on the candidates:
...
1. Catalog Service receives GET /v1/catalog?dc_id=PHL-12&category=snacks&q=chips&page=1. It queries the Product table for matching SKUs, then bulk-fetches availability from Redis:
...
1. Elasticsearch runs a multi_match against name, brand, and category fields with category boosting (exact matches rank higher). The query is scoped to is_active=true:
...
1. Order Service checks idempotency: SELECT status, created_at FROM orders WHERE order_id = ?. If found, return the existing order state immediately — no reprocessing.
1. Order Service calls Inventory Service: POST /v1/inventory/reserve with {dc_id, order_id, items: [{product_id, quantity}]}. Inventory Service runs a transactional reserve:
...
1. As the picker scans each line item, the picking app sends POST /v1/fulfillment/scan with {order_id, product_id, actual_product_id?, substitution_reason?}. Fulfillment Service:
...
1. Fulfillment Service queries Catalog Service for substitution candidates: GET /v1/catalog/substitutions?product_id=4821&dc_id=PHL-12. Catalog Service returns the top 3 candidates ranked by:
...
1. Inventory Service consumes OrderCreated, reserves items, publishes InventoryReserved or InventoryUnavailable.
...
1. [Uber: Evolution and Scale of Uber's Delivery Search Platform — two-tower semantic search, HNSW indexing, biweekly index refresh, blue/green column-level deployment for catalog updates](https://www.uber.com/us/en/blog/evolution-and-scale-of-ubers-delivery-search-platform/)
1. [Uber: Cart Assistant — Agentic Grocery Shopping on Uber Eats — multi-prompt state graph with 8 stages, parallel item processing, guardrail architecture for product recommendations](https://www.uber.com/us/en/blog/uber-cart-assistant/)
1. [PostgreSQL: SELECT FOR UPDATE and Row-Level Locks — row-level locking semantics, deadlock detection, lock ordering](https://www.postgresql.org/docs/current/explicit-locking.html)
1. [Redis: Pipelining and Hashes for Bulk Reads — HMGET for batch field retrieval, pipeline batching for snapshot loads](https://redis.io/docs/latest/develop/use/pipelining/)
1. [Kafka: Exactly-Once Semantics and the Outbox Pattern — transactional outbox, idempotent producers, consumer deduplication](https://www.confluent.io/blog/exactly-once-semantics-are-possible-heres-how-apache-kafka-does-it/)
1. [OSRM: Open Source Routing Machine — Table Service — distance/duration matrices for candidate routing at < 50ms per query](https://project-osrm.org/docs/v5.24.0/api/#table-service)
Comparison sources

- [DoorDash: Building a Scalable Search and Discovery Platform — menu availability caching, geo-search partition strategy](https://doordash.engineering/)
- [Instacart: Real-Time Inventory and Fulfillment Architecture — reservation TTLs, substitution logic, picker workflows](https://tech.instacart.com/)
- [GoPuff Engineering Blog — micro-fulfillment center operations, real-time order tracking](https://medium.com/gopuff-engineering)
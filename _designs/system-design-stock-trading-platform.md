...
graph LR
    ...
    subgraph "Risk Engine (in-memory)"
        RE[Risk Engine]
        CACHE[("Cache<br/>buying_power<br/>day_trades")]
    end
    subgraph "Database"
        DB[("PostgreSQL")]
    end

    OS -->|"1. CheckRisk(user_id, order)"| RE
    RE -->|"2. Lookup"| CACHE
    CACHE -.->|"buying_power, day_trades"| RE
    RE -->|"3a. REJECT"| OS
    RE -->|"3b. ACCEPT (reserved)"| OS
    OS -->|"4. BEGIN; SELECT...FOR UPDATE"| DB
    DB -->|"Re-validate"| OS
    OS -->|"5. INSERT/UPDATE/COMMIT"| DB
    OS -->|"6a. ConfirmRisk(success=true)"| RE
    OS -->|"6b. ConfirmRisk(success=false)"| RE
    RE -->|"Rollback cache"| CACHE
```
